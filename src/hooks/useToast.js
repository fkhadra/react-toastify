...
export function useToast(props) {
    ...
    var _b = useState(false), preventExitTransition = _b[0], setPreventExitTransition = _b[1];
    ...
    function onDragTransitionEnd(e) {
        var _a = toastRef.current.getBoundingClientRect(), top = _a.top, bottom = _a.bottom, left = _a.left, right = _a.right;
        if (e.nativeEvent.type !== 'touchend' &&
            props.pauseOnHover &&
            e.clientX >= left &&
            e.clientX <= right &&
            e.clientY >= top &&
