...
function TestComponent(props) {
  ...
  var _b = React.useState(''),
    updateId = _b[0],
    setUpdateId = _b[1];
  var _c = useNotificationCenter(props || {}),
    unreadCount = _c.unreadCount,
    markAllAsRead = _c.markAllAsRead,
    markAsRead = _c.markAsRead,
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        {
          onClick: function () {
            return update(updateId, { content: content });
          }
        },
        'updateNotification'
      )
    ...
    React.createElement('input', {
      ...
      type: 'text',
      onChange: function (e) {
        return setUpdateId(e.target.value);
      },
      value: updateId
    }),
    React.createElement(
      ...
      notifications.map(function (el) {
        return React.createElement(
          'li',
          { key: el.id, style: flex },
          React.createElement(
            'span',
            { 'data-testid': 'content-'.concat(el.id) },
            el.content
          ),
          React.createElement(
            'span',
            { 'data-testid': 'read-'.concat(el.id) },
            el.read.toString()
          ),
          React.createElement(
            'button',
            {
              'data-testid': 'markAsRead-'.concat(el.id),
              onClick: function () {
          ...
          React.createElement(
            'button',
            {
              'data-testid': 'remove-'.concat(el.id),
              onClick: function () {
                return remove(el.id);
              }
            },
