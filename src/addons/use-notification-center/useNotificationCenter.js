...
export function useNotificationCenter(params) {
    ...
    useEffect(function () {
        return toast.onChange(function (item) {
            if (item.status === 'added' || item.status === 'updated') {
                ...
                if (filterFn.current && !filterFn.current(newItem_1))
                    return;
                setNotifications(function (prev) {
                    var nextState = [];
                    var updateIdx = prev.findIndex(function (v) { return v.id === newItem_1.id; });
                    if (updateIdx !== -1) {
    ...
    var remove = function (id) {
        setNotifications(function (prev) {
            return prev.filter(Array.isArray(id) ? function (v) { return !id.includes(v.id); } : function (v) { return v.id !== id; });
        });
    };
    var clear = function () {
        setNotifications([]);
    ...
    var markAsRead = function (id, read) {
        ...
        setNotifications(function (prev) { return prev.map(map); });
    };
    var find = function (id) {
        return Array.isArray(id)
            ? notifications.filter(function (v) { return id.includes(v.id); })
            : notifications.find(function (v) { return v.id === id; });
    };
    var add = function (item) {
        if (notifications.find(function (v) { return v.id === item.id; }))
            return null;
        var newItem = decorate(item);
        setNotifications(function (prev) { return __spreadArray(__spreadArray([], prev, true), [newItem], false).sort(sortFn.current); });
        return newItem.id;
    };
    var update = function (id, item) {
        var index = notifications.findIndex(function (v) { return v.id === id; });
        if (index !== -1) {
            setNotifications(function (prev) {
    ...
    var sort = function (compareFn) {
        ...
        setNotifications(function (prev) { return prev.slice().sort(compareFn); });
    };
    return {
        ...
        sort: sort,
        get unreadCount() {
            return notifications.reduce(function (prev, cur) { return (!cur.read ? prev + 1 : prev); }, 0);
        }
    };
}
