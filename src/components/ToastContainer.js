...
export function ToastContainer(props) {
  ...
  var _b = useToastContainer(containerProps),
    getToastToRender = _b.getToastToRender,
    isToastActive = _b.isToastActive,
    count = _b.count;
  var className = containerProps.className,
    style = containerProps.style,
    rtl = containerProps.rtl,
    containerId = containerProps.containerId;
  function getClassName(position) {
    var _a;
    var defaultClassName = cx(
      ''.concat('Toastify' /* Default.CSS_NAMESPACE */, '__toast-container'),
      ''
        .concat('Toastify' /* Default.CSS_NAMESPACE */, '__toast-container--')
        .concat(position),
      ((_a = {}),
  ...
  useIsomorphicLayoutEffect(
    function () {
      ...
      if (stacked) {
        ...
        var gap_1 = 12;
        var isTop_1 =
          (_a = containerProps.position) === null || _a === void 0
            ? void 0
            : _a.includes('top');
        var usedHeight_1 = 0;
        var prevS_1 = 0;
        Array.from(nodes)
          ...
          .forEach(function (n, i) {
            ...
            if (i > 0) node.dataset.collapsed = ''.concat(collapsed);
            if (!node.dataset.pos) node.dataset.pos = isTop_1 ? 'top' : 'bot';
            var y =
              usedHeight_1 * (collapsed ? 0.2 : 1) +
              (collapsed ? 0 : gap_1 * i);
            node.style.setProperty(
              '--y',
              ''.concat(isTop_1 ? y : y * -1, 'px')
            );
            node.style.setProperty('--g', ''.concat(gap_1));
            node.style.setProperty(
              '--s',
              ''.concat(1 - (collapsed ? prevS_1 : 0))
            );
  ...
  return React.createElement(
    ...
    {
      ref: containerRef,
      className: 'Toastify' /* Default.CSS_NAMESPACE */,
      id: containerId,
    ...
    getToastToRender(function (position, toastList) {
      var containerStyle = !toastList.length
        ? __assign(__assign({}, style), { pointerEvents: 'none' })
        : __assign({}, style);
      return React.createElement(
        'div',
        {
          ...
          key: 'container-'.concat(position)
        },
        toastList.map(function (_a) {
          ...
          return React.createElement(
            ...
            __assign({}, toastProps, {
              ...
              key: 'toast-'.concat(toastProps.key)
            }),
            content
          );
        })
