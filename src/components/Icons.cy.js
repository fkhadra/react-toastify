...
describe('Icons', function () {
    it('handle function', function () {
        var C = getIcon(__assign(__assign({}, props), { icon: function () { return React.createElement("div", null, "icon"); } }));
        cy.mount(C);
        cy.findByText('icon').should('exist');
    });
    it('handle react element', function () {
        var C = getIcon(__assign(__assign({}, props), { icon: React.createElement("div", null, "icon") }));
        cy.mount(C);
        cy.findByText('icon').should('exist');
    });
    ...
    it('handle built-in icons', function () {
        for (var _i = 0, _a = ['info', 'warning', 'success', 'error', 'spinner']; _i < _a.length; _i++) {
            var t = _a[_i];
